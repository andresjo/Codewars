...
the method returns an array containing a single string: "Empty" (or Nothing in Haskell, or "None" in Python)
...
class WordSearch {
	  static String[] findWord(String x, String[] y){
		  Object[] res  = Arrays.stream(y)
		  		.filter(s -> s.toLowerCase().indexOf(x) > -1)
		  		.toArray();
